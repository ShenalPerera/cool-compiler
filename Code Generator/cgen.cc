...
CgenClassTable::CgenClassTable(Classes classes, ostream &s) : nds(NULL), str(s)
{
  ...
  for (List<CgenNode> *li = nds; li; li = li->tl())
  {
    classes_vector.push_back(li->hd());
  }
...
void CgenClassTable::code()
{
  ...
  if (cgen_debug)
    cout << "coding class initializing methods" << endl;
  code_obj_init();
...
// To dispatch all arguments on the stack
...
{
  // To generate code for each expression
...
void CgenClassTable::code_class_nameTab()
{
  ...
  // Iterating over all classes in the vector
...
void CgenClassTable::code_objTab()
{
  ...
  for (int i = classes_vector.size() - 1; i > -1; i--)
  {
    str << WORD << classes_vector[i]->get_name() << PROTOBJ_SUFFIX << endl
        << WORD << classes_vector[i]->get_name() << CLASSINIT_SUFFIX << endl;
  }
}

// Function to dispatch tables
void CgenClassTable::make_dispTab(CgenNode *n, Symbol current_class, std::vector<Symbol> *met_ordered)
{
  if (n->get_name() != Object)
  {
    make_dispTab(n->get_parentnd(), current_class, met_ordered);
  }
  for (int i = n->features->first(); n->features->more(i); i = n->features->next(i))
  {
    method_class *M = dynamic_cast<method_class *>(n->features->nth(i));
    if (M != NULL)
    {
      std::map<Symbol, std::pair<int, Symbol>> methodList = dispTab[current_class];
      if (methodList.find(M->name) != methodList.end())
      {
        methodList[M->name].second = n->get_name();
        dispTab[current_class] = methodList;
      }
      else
      {
        dispTab[current_class].insert(std::pair<Symbol, std::pair<int, Symbol>>(M->name, std::pair<int, Symbol>(dispTab[current_class].size(), n->get_name())));
        (*met_ordered).push_back(M->name);
      }
    }
  }
}
...
{
  List<CgenNode> *li = nds;
  while (li)
  {
    Symbol class_name = (li->hd())->get_name();
    std::vector<Symbol> v;
    make_dispTab(li->hd(), class_name, &v);
    std::map<Symbol, std::pair<int, Symbol>> methodList = dispTab[li->hd()->get_name()];
    str << class_name << DISPTAB_SUFFIX << LABEL;
    for (unsigned i = 0; i < v.size(); i++)
    {
      Symbol met_name = v[i];
      str << WORD << methodList[met_name].second << METHOD_SEP << met_name << endl;
    }
    li = li->tl();
  }
}

// Prototype objects function
...
{
  for (unsigned int i = 0; i < classes_vector.size(); i++)
  {
